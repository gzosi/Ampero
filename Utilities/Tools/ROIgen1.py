...
def select_point(event, x, y, flags, param):
    ...
    # Accettiamo sia il click singolo che il doppio click rapido
    if event == cv2.EVENT_LBUTTONDOWN or event == cv2.EVENT_LBUTTONDBLCLK:
        
        # Filtro Anti-Rimbalzo: controlla se il punto è identico/troppo vicino al precedente
        if len(current_points) > 0:
            last_x, last_y = current_points[-1]
            # Se il nuovo clic è a meno di 3 pixel di distanza dal precedente, ignoralo
            if abs(x - last_x) < 3 and abs(y - last_y) < 3:
                return # Esce senza aggiungere il punto
